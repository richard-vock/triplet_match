...
identity_traits<Point>::init_from_samples(
    const const_handle_t&, const samples_t& samples) {
    auto handle = std::make_shared<state_t>();
    return handle;
}
...
identity_traits<Point>::project(const const_handle_t& handle, const vec3f_t& xyz) {
    return xyz;
}

template <typename Point>
inline vec3f_t
identity_traits<Point>::unproject(const const_handle_t& handle, const vec3f_t& uvw) {
    return uvw;
}

template <typename Point>
inline vec3f_t
identity_traits<Point>::tangent(const const_handle_t& handle, const Point& pnt) {
    return vec3f_t(pnt.data_c[1], pnt.data_c[2], pnt.data_c[3]);
}

template <typename Point>
inline vec3f_t
identity_traits<Point>::normal(const const_handle_t& handle, const Point& pnt) {
    return pnt.getNormalVector3fMap();
}

template <typename Point>
inline float
identity_traits<Point>::intrinsic_distance(const const_handle_t& handle,
                                        const vec3f_t& p0,
                                        const vec3f_t& p1) {
    return (p1 - p0).norm();
}
